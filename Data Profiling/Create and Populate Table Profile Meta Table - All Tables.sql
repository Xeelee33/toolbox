/*
Script: Create and Populate Table Profile Meta Table - All Tables.sql
Created by: Joshua Wilshere
Created on: 9/23/24

Purpose: Creates, or if exists, populates a schema and table called meta.TABLES_PROFILES with a basic profile of all tables in the database

Instructions: 
	1. If needed, add a USE YOUR_DATABASE_NAME; directly below this commend block and above the DECLARE statement
	2. Exclude any tables you don't wnat to profile from the ##tableListTable initilization query on line 48
	5. Execute
...
IF OBJECT_ID('tempdb..##tableListTable') IS NOT NULL DROP TABLE ##tableListTable;
...
-- Initialize ##tableListTable with list of all schemas and tables
...
where s.name != 'meta'
and t.name != 'TABLE_PROFILES'


/*************** EXECUTE SCRIPT ***************/

-- Loop through ##tableListTable and execute profiling script for each schema.table in ##tableListTable

WHILE EXISTS (SELECT 1 FROM ##tableListTable)
BEGIN
	SELECT TOP 1
	...
	-- Delete already profiled schema.table from ##tableListTable
	DELETE ##tableListTable
